...
const Contact = () => {
  ...
  useEffect(() => {
    const q = query(collection(db, 'contact_details'), orderBy('sort_order'))
    const unsub = onSnapshot(q, (snap) => {
      setContactDetails(snap.docs.map(d => ({ id: d.id, ...d.data() } as ContactDetail)))
    })
    return () => unsub()
  }, []);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`text-center mb-16 transition-all duration-1000 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
            Ready to join our adventures? Have questions about membership or upcoming events? We would love to hear from you.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <div className={`transition-all duration-1000 delay-300 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
                  ...
                  return (
                    <div key={detail.id} className="flex items-start space-x-4">
                      ...
                      <div>
                        ...
                        {detail.sub_info && <p className="text-gray-600 text-sm">{detail.sub_info}</p>}
                      </div>
                    </div>
                  );
                })
            ...
            <div className={`mt-12 p-8 bg-white rounded-2xl shadow-lg transition-all duration-1000 delay-900 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
